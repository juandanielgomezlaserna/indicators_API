...
const obtenerResumenGeneralService = async (usuarioId) => {
  const [
    ...
    // 2. Logros (Corregido idIndicador y creado_at según tu estructura real)
    pool.query(
    ...
    // 5. Cartera: Metas (Removido fecha_limite que no existe en esta tabla)
    pool.query(
      'SELECT id, nombre, monto_actual, monto_objetivo, completado, created_at FROM public.cartera_metas WHERE usuario_id = $1::uuid',
      [usuarioId]
    ),
    
    // 6. Cartera: Deudas (Usando fecha_limite_pago que sí existe)
    pool.query(
